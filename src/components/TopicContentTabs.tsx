...
interface TopicContentMaterial {
  id: string;
  title?: string;
  description?: string;
  url: string;
...
interface TopicContentAssignment {
  id: string;
  title?: string;
  description?: string;
  dueDate?: string;
...
interface TopicContentLiveClass {
  id: string;
  title?: string;
  description?: string;
  scheduledAt?: string;
...
interface TopicContentLibraryResource {
  id: string;
  title?: string;
  description?: string;
  resourceType?: string;
...
interface TopicContentTabsProps {
  ...
  onAddQuiz?: () => void;
  onVideoPress?: (video: TopicContentVideo) => void;
  onMaterialPress?: (material: TopicContentMaterial) => void;
  onRefresh?: () => void;
}
...
export function TopicContentTabs({ 
  ...
  const {
    ...
    queryKey: ['topicContent', topic.id, userRole],
    queryFn: () => userRole === 'teacher' 
      ? ApiService.teacher.getTopicContent(topic.id)
      : ApiService.student.getTopicContent(topic.id),
    enabled: !!topic.id,
  ...
  const handleVideoPress = (video: TopicContentVideo) => {
    ...
    if (userRole === 'teacher') {
      ...
      (navigation as any).navigate('StudentVideoPlayer', { 
        videoUri: video.url,
        videoTitle: video.title,
        videoDescription: video.description,
        topicTitle: topicTitle || 'Untitled Topic',
      ...
      // Director can view videos but not edit - for now just log
  ...
  const handleMaterialPress = (material: TopicContentMaterial) => {
    ...
    // Default behavior - just show an alert
      ...
      {videos.length > 0 ? (
        ...
        <View className="items-center py-6">
          ...
          <Text className="text-xs text-gray-400 dark:text-gray-500 text-center mt-1">
            {userRole === 'teacher' ? 'Start by adding your first video lesson' : 'Videos will appear here when available'}
          </Text>
        </View>
      )}
    </View>
    ...
    <View className="space-y-3">
      <View className="flex-row items-center justify-between mb-4">
        ...
        <View className="flex-row items-center gap-2">
          ...
          {userRole === 'teacher' && onAddMaterial && (
            <Pressable
      ...
      {materials.length > 0 ? (
        ...
        <View className="items-center py-6">
          ...
          <Text className="text-xs text-gray-400 dark:text-gray-500 text-center mt-1">
            {userRole === 'teacher' ? 'Add study materials, PDFs, and other resources' : 'Materials will appear here when available'}
          </Text>
        </View>
      )}
    </View>
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'assignments':
        return <View><Text>Assignments coming soon...</Text></View>;
      case 'others':
        return <View><Text>Other content coming soon...</Text></View>;
      default:
        return null;
    }
  };
