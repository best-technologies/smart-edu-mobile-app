...
import { directorService, Student } from '@/services/api/directorService';
import EmptyState from '../../components/shared/EmptyState';

export default function AllStudentsListScreen() {
  ...
  const fetchStudents = useCallback(async (page: number = 1, search: string = '', isLoadMore: boolean = false) => {
    try {
      if (isLoadMore) {
      ...
      const response = await directorService.fetchStudentsData(params);
  ...
  // Fetch students when debounced search query changes
  useEffect(() => {
    ...
    if (debouncedSearchQuery === '' || debouncedSearchQuery.length >= 3) {
      ...
      // If query is less than 3 characters, show all students
      fetchStudents(1, '');
    }
  ...
  const handleLoadMore = () => {
    if (!isLoadingMore && hasMore) {
      fetchStudents(currentPage + 1, searchQuery, true);
    }
  };
  ...
  const renderStudentItem = ({ item, index }: { item: Student; index: number }) => {
    const initials = `${item.first_name.charAt(0).toUpperCase()}${item.last_name.charAt(0).toUpperCase()}`;
          ...
          <View className="flex-row items-center justify-between mb-3">
            <View className="flex-row items-center flex-1">
              ...
              <View className="flex-1">
                ...
                <Text className="text-sm text-gray-500 dark:text-gray-400 font-mono" numberOfLines={1}>
                  {item.student_id}
                </Text>
              </View>
            </View>
        ...
        <View className="mt-3">
          <Text className="text-sm text-gray-500 dark:text-gray-400">
            {isLoading ? 'Loading...' : 
             ...
             `${totalResults} students found${debouncedSearchQuery ? ` for "${debouncedSearchQuery}"` : ''}`
            }
          </Text>
        </View>
      </View>
